# Contributing to ep_list_pads

Thanks for helping improve `ep_list_pads` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_list_pads` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_list_pads from your local clone
pnpm run plugins i --path ../ep_list_pads

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_list_pads/` are picked up after a server restart.
...
* Etherpad version, ep_list_pads version, browser + OS
* Steps to reproduce
