# Agent Guide — ep_list_pads

List Pads on the Index Page.
...
`ep_list_pads` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_list_pads`:
...
* Server: `expressCreateServer`, `eejsBlock_indexWrapper`
